 ...
 */
public class ServicesRunActor extends AbstractActor {

    @Inject
    public ServicesRunActor(ThemeService itemService, WSClient ws, Configuration configuration) {
            ...
            if (versionVoList != null && versionVoList.size() > 0) {
                ...
                F.Promise<File> filePromise = ws.url(configuration.getString("zip.download.url") + URLEncoder.encode(versionVo.getDownloadLink(), "UTF-8")).get().map(response -> {
                    ...
                    try {
                        ...
                        String zipPath = configuration.getString("services.zip.path");

                        final File file = new File(zipPath);

                        Logger.error("services.zip.path文件为: " + file.getPath());
